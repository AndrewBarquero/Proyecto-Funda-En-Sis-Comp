...
try:
    ...
    ser = None
    print("ERROR: No se pudo abrir COM3:", e)
...
def modo_de_juego():
    ...
    def transmision():
        ...
        fase_actual = 0
        palabra_actual = random.choice([p for p in palabras if len(p) <= 16])
        puntaje_total = 0
    ...
    tk.Button(
        fondo,
        text="Modo Escucha vs Transmisión",
        bg="RoyalBlue4",
        fg="White",
        width=25,
        command=lambda: modo_vs(ventana_juego1)
    ).place(relx=0.27, rely=0.45)
    tk.Button(
        fondo,
        text="Volver",
        bg="RoyalBlue4",
...
# ---------------------- MODO ESCUCHA VS TRANSMISIÓN ----------------------
def modo_vs(ventana_juego1):
    ventana_juego1.withdraw()

    ventana_vs = tk.Toplevel(ventana_principal)
    ventana_vs.title("Modo Escucha vs Transmisión")
    ventana_vs.geometry("520x420+650+200")
    ...
    entrada_vs = tk.Entry(ventana_vs, width=25, font=("Arial", 12))
    entrada_vs.pack(pady=10)
    ...
    def iniciar():
        ...
        if ser:
            ser.write((palabra + "\n").encode())
        texto.config(text="Escucha el mensaje...\nJugador A escribe primero")
        turno = "A"

    def evaluar():
        nonlocal puntaje_A, puntaje_B, turno
        respuesta = entrada_vs.get().upper()
        entrada_vs.delete(0, tk.END)

        aciertos = sum(
            1 for i in range(min(len(palabra), len(respuesta)))
        ...
        if turno == "A":
            puntaje_A = aciertos
            turno = "B"
            texto.config(text="Turno Jugador B\nUsa el botón físico")
            if ser:
                ser.write(b"TURNO=B\n")
        else:
            puntaje_B = aciertos
    ...
    ventana_vs.protocol("WM_DELETE_WINDOW", volver)

    tk.Button(ventana_vs, text="Start", bg="RoyalBlue4", fg="White", command=iniciar).pack(pady=5)
    tk.Button(ventana_vs, text="Enviar respuesta", bg="RoyalBlue4", fg="White", command=evaluar).pack(pady=5)
    tk.Button(ventana_vs, text="Volver", bg="RoyalBlue4", fg="White", command=volver).pack(pady=15)
# ===================== BOTONES PRINCIPALES =====================
tk.Button(
    imagen_inicio,
    text="Jugar",
